...
v2_select_start() {
  ...
  if (( ! v2_start_seen )); then
    if [[ ! -t 0 ]]; then
      echo '시작 CSV를 선택해야 합니다. start_waypoint:=01 등으로 01~07 중 하나를 지정하세요.' >&2
      return 2
    fi
    echo '시작 CSV 선택: 등록된 경로 01 / 02 / 03 / 04 / 05 / 06 / 07' >&2
    while :; do
  ...
  case "$v2_start" in
    0[1-7]) ;;
    *) echo 'start_waypoint는 01~07 중 하나여야 합니다.' >&2; return 2 ;;
  esac
  if (( ! v2_end_seen )); then
    # 06 and 07 are alternative terminal branches, not consecutive legs.
    v2_end=07
    [[ "$v2_start" != 06 ]] || v2_end=06
  ...
  if [[ ( "$v2_start" == 06 || "$v2_start" == 07 ) && "$v2_start" != "$v2_end" ]]; then
    echo '06 또는 07에서 시작하면 종료 경로도 같은 번호여야 합니다.' >&2
    return 2
  fi
