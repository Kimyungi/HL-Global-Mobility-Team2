...
> 이 파일은 확정 아키텍처 설계 결론을 담는다. Claude Code는 모든 세션에서 이 문서를 프로젝트의 기준으로 삼을 것.
...
계층 모델 (교수님 확정): **Signal processing → Decision → Control → Actuation**
...
- dSPACE는 multi-rate: 10ms 태스크(Vehicle MGM)와 5ms 태스크(Actuation)가 한 프로세서에서 병행. **주기는 합산되지 않음** — 10ms 틱 안에서 수신→궤적→MPC가 연속 실행.
...
| flags | uint | state + packet counter. **counter는 watchdog 판정 입력** |
...
**watchdog (dSPACE):** 패킷 counter가 30ms(TX 3주기) 동안 미갱신 → v_ref = 0 (감속 정지), 조향은 직전 값 유지(급조향 금지). 타임아웃 값은 §7 검증 결과에 따라 조절 가능(예: 50ms).
...
| avoid → 복귀 | 기동 완료 → **직전하던 스테이트로** 복귀 (복귀처 변수 1개만 기억) |
...
1. **조립/병합 블록에 판단 로직 금지.** ref points 조립·종방향 병합은 스테이트의 결정을 포맷 변환·후처리하는 순수 실행부. `if (조건) v_ref = 0` 같은 조건 분기를 여기 넣는 순간 판단이 두 곳으로 흩어진다. 모든 "무엇을 할지"는 스테이트 머신으로.
...
4. **모든 경로 소스는 동일 ref points 포맷.** 차선/GPS/회피/주차 어느 것이 오건 dSPACE는 구분할 필요 없음 → MPC 무상태 원칙.
5. **PWM 주파수 혼동 금지.** 서보 = 50Hz(펄스폭이 위치), 구동 모터 드라이버 = 20kHz(duty가 전압).
6. 스테이트 전환 시 ref 불연속 방지(전환 연속 처리), 급격한 v_ref 변화 rate limit — 조립 블록의 허용 임무.

## 6. 워크스페이스 구조

```
FMA_ws/src/
├── fma_interfaces/        # msg/인터페이스 정의 — 모든 스택의 공용 계약 (§3 포맷)
│                          #   (이름 주의: common_interfaces는 ROS 2 코어 메타패키지와 충돌하여 fma_interfaces로 확정)
├── bridge_dspace/         # Ethernet UDP 브리지 (TX/RX) — ★ 최우선 구현, 배포 전 완성
├── adas_mgm/              # Decision — 10ms 루프, 스테이트 머신, 조립/병합, 지터 로깅
...
- 각 stack 폴더는 `REQUIREMENTS.md` 포함 (담당자별 요구사항).
- 각 스택의 출력은 `fma_interfaces`에 정의된 토픽/메시지로만 — MGM은 그 토픽들만 구독.
- 부트스트래핑 순서: ① bridge_dspace로 PC↔dSPACE 왕복 검증 (더미 ref로 바퀴 반응 + vehicle vector 회신 확인) → ② adas_mgm 10ms 루프 골격 + 지터 로깅 → ③ 각 스택 배포·병렬 개발.
...
- **판정식: "최악 지연 × 2를 watchdog 타임아웃으로 잡았을 때, 그 타임아웃이 안전한가(PC 사망 시 정지 개시까지의 이동거리가 수용 가능한가)?"**
  - Yes → v1 유지 (필요 시 타임아웃 조절, 예: 30→50ms)
  - No (지연 꼬리가 제고되지 않음) → v3 이관
...
| 손상민 | 라이다 주차 + MPC·Vehicle MGM (dSPACE 전용, 검증은 카메라·GPS 주행 연계) | 주차 단독 + MPC 검증 |
| 이기돈 | 장애물 회피 + 하위 제어 보수(김재민에게서 인수) | 회피 단독 주행 |
| 김재민 | 신호등·정지선 + 하위제어 사전(기반 7/6~7/17 완료) | 신호등 정지 |
| 박찬미 | 돌발 장애물 긴급 정지 | 긴급 정지 |

미팅 주 2회(월·목). 마일스톤: 8/10 일정 단독 주행 통합 시연.
