...
- **avoid** — 기동 완료 우선, 정지 요구는 기동 이탈 후 적용. 단 TTC < 임계 → 즉시 정지(안전 바닥). 횡: 회피 경로 / 종: v_ref는 회피 기하로 결정(여유 폭 좁으면 감속). **통과 대기 구간의 헤딩은 GPS 트랙 기준으로 규제한다 (2026-08-16 신설)**: stack_avoid가 장애물을 현재 감지하지 않을 때(=통과 대기, 유지점 (1.5,0) 발행 구간 — 실측 일치율 100%) 그 목표를 **트랙 헤딩 오차 ψₑ(`gps_path.pts[0].yaw`)만큼 회전**시켜 발행한다(±25° 클램프, `gps_path` 유효 시에만). 근거: AVOID의 "전방 직진 유지"는 **트랙 기준 헤딩이 개루프**라 dSPACE 잔류 조향이 그대로 적분된다 — 실측 AVOID 회당 헤딩 24~42° 누적·횡오차 0.24~1.10m 증가, 그 오차를 그대로 안고 GPS로 복귀해 복귀 구간이 ±50~93° 스윙하며 불안정해졌다(run_0816_174556·175102, AVOID 5회 전수). 정보는 이미 있었다 — AVOID 중 `gps_path` 유효율 100%, 통과 대기 구간이 전체의 70~89%(7~10.7초). **횡방향으로는 트랙에 끌어당기지 않는다** — 트랙 헤딩에 평행하게만 맞춘다(옆의 장애물로 파고드는 것 방지). 횡 복귀는 통과 후 GPS 재합류가 담당. ψₑ가 줄면 회전량도 0으로 수렴하는 자기 종결형이다
