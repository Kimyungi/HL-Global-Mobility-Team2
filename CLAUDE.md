...
- **traffic** — 횡방향은 lane 경로 유지. **2026-09-01 개정(단순화)**: 처음엔 적색 진입 시점의 최초 유효 거리만 래치해 고정 제동곡선(`v=sqrt(2·a·remaining)`)을 따라가는 설계였는데, 정지선 인식이 간헐적으로만 성공하는 조건(같은 날 해질녘 실측: YOLO/HSV 후보조차 못 찾는 완전 실패 포함)에서는 그 최초 한 번의 관측이 아예 없거나 부정확할 위험이 커서, 검증이 더 필요한 지금 단계는 디버깅이 쉬운 단순 비율 방식으로 되돌렸다(팀 논의). `stop_distance`가 안정 검출되는 **매 틱마다** `traffic_stopline_distance`를 그 값으로 계속 갱신하고(최초 값에 고정하지 않음), 안 보이는 동안만 dSPACE→PC `/vehicle/vector.v`로 dead-reckoning 감쇠한다. `remaining = traffic_stopline_distance − traffic_stop_offset_m`에 대해 `v_ref = clamp01(remaining / traffic_ramp_distance_m) × v_base`(기본 ramp 거리 1.5m). **정지선을 한 번도 못 봤으면(=거리를 모르면) 즉시 0으로 정지** — v_base로 계속 달리다 신호를 지나치는 사고를 막기 위한 안전 쪽 폴백(구 설계는 이 경우 v_base 유지였다). traffic 또는 실차속도 입력이 stale이면 fail-safe 정지. 확정 초록만 상태를 해제하며 미검출/unknown은 유지 근거다. 생성 `ADAS_MGR2 v1.88`에는 이 상태가 없으므로 `traffic_state_enabled=true`에서는 generated backend 기동을 거부하고, 모델 재생성 전까지 production core만 사용한다. bridge_dspace/tools/camera_traffic_ref_test.py로 벤치에서 먼저 검증한 방식. **실차 미검증** — 카메라 장착 고정 후 `traffic_ramp_distance_m` 재보정 권장.
