...
- **lane · waypoint** — 종방향: 긴급 정지 > 신호등 정지 > 트랙 종점 정지(waypoint만, `GpsPath.at_end` — 종점 통과 시 ref가 차량 뒤로 가 유턴하는 것 방지, 2026-08-03 주행에서 확인) > 역방향 정지(waypoint만 — |ref[0].yaw| > 120°가 0.5s 지속 = 차가 경로를 등짐. 유턴 후 트랙을 거꾸로 재추종하는 것 차단, 2026-08-03 2회 재현) > 가속구간 > 기본 속도 / 횡방향: 차선 경로 / GPS 경로
