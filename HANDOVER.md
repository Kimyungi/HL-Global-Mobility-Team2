...
> **2026-08-29 갱신 — 지금은 허브 하나로 운용한다.** 카메라의 **전원·통신을 별도 라인으로
> 분리**해서 아래 전류 경합의 원인 자체를 없앴기 때문이다. 허브를 가르는 것은 수단이었지
> 목적이 아니었다. `check_sensors.py` 의 허브 항목도 같은 날 판정 → 경고로 내렸다.
> 아래 함정 설명은 **증상 판정법으로 계속 유효하다** — 카메라를 다시 버스 전원으로
> 물리는 순간 그대로 재현된다. 현재 배치도는 `multi_lidar_fusion/tools/99-fma-lidars.rules`
> 머리주석이 정본이다.

**전류를 갈라야 하는 이유(이력).** OAK-D 2대(각 500mA 신고)를 라이다와 같은 허브에 꽂으면
...
4대가 즉시 10Hz 로 살아났다. (2026-08-29 부터는 카메라 전원을 별도 라인으로
빼서 같은 결과를 얻는다.) (좌측이 `SL_RESULT_OPERATION_TIMEOUT` 으로 죽는 형태로
나타나기도 한다.)
