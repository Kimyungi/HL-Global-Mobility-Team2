...
유지하며, 출구 검출기는 `image_topic:=/perception/lane_image_raw`로 라인
카메라의 원본 영상을 구독한다. 출구 분기가 있으면 `lane_enabled:=true`가 필요하다.
라인 추론이 구역에 의해 꺼져도 원본 영상은 계속 발행한다. 신호등 판단만 구역 제한하며 카메라를 재연결하지 않는다.
...
접근 단계에는 일반 내비게이션 속도를 유지한다. APPROACH부터 출구 판별 종료까지 출구 검출기는 라인 카메라의 원본 영상 토픽을 사용하며, 카메라 소유권을 전환하지 않는다. 실행 중 GPS/MGM/검출기에는 메시지 필드가 추가되었으므로 전체 런처를 다시 시작해야 한다.
