...
> **해결 (2026-08-06):** 위빙의 주원인은 dSPACE 게인이 아니라 **PC측 ref 첫 점
> 기하**로 확정. dSPACE가 첫 점만 목표로 쓰는데(이기돈 실측) 우리가 최근접점
> (차 옆구리, x≈0)을 줘서 도달 곡률이 폭주 — lookahead 1m 적용 후 같은 직선에서
> **cross 평균 0.38→0.075m, 최대 1.64→0.24m, 완주+종점 자동정지**. 아래 요청 중
> 서보 스텝응답·MPC 지연 반영은 여전히 유효(성능 향상 여지), 우선순위만 하락.
> 확인 요청 유지: ① 0x101~ 다점 중 실제로 몇 점을 쓰는지(계약 정리) ② str 부호
> ③ counter 미갱신 ④ 최소 회전 반경.
