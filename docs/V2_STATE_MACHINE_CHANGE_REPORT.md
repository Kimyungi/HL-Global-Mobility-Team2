...
| 10–11, 47–50 | 변경 | [신호 모듈 준비](../src/stack_traffic/stack_traffic/zone_supervisor.py) | 상시 실행 및 메시지 소실에 의한 별도 정지 | 카메라·영상 발행은 zone 밖에서도 상시 유지. 신호등·정지선 추론 및 판단 메시지는 확인된 traffic zone 안에서만 활성화. 출발 시 zone 밖 heartbeat 요구 제외 및 시작 준비 timeout 없음 정책 유지. |
...
| 47, 53–54 | 변경 | [신호 모듈 생명주기](../src/stack_traffic/stack_traffic/zone_supervisor.py) | GPS zone과 독립적으로 인식 프로세스 상시 실행 | 2026-09-18 변경: 프로세스·카메라·모델은 상시 유지하고, 확인된 공용 turn zone에서만 판단 실행. 진입·이탈 시 적색·투표·거리·추적 이력을 초기화. zone 밖에서도 RViz 카메라 영상은 계속 갱신하며, 실제 카메라 고장 래치는 zone 전환으로 해제하지 않음. |
