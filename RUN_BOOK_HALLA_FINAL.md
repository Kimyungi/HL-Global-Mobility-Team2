...
명시한 zone ID는 시작 경로와 관계없이 유지합니다. zone [3]에서 현재 신호등·정지선
모듈을 활성화하고 전진 목표속도를 최대 1m/s로 제한합니다. zone [2]는 별도 신호등
구역으로 구분하며 이 모듈과 속도 제한을 적용하지 않습니다. 자동 생성되는 주차 구역 ID는
명시된 ID를 피해 할당하고, 주차 임무 완료 식별자는 경로 전체에서 고유하게 유지합니다.
...
출발 준비 조건은 **라이다 4대 정상 AND GPS FIXED 준비**입니다.
라인 추론과 라인 주행은 사용하지 않습니다. 카메라 원본 영상은 마지막 미션에 제공합니다.
...
시작 경로는 01~07 중 선택하며, 생략하면 터미널에서 질문합니다.
경로 05를 포함하면 06·07을 함께 사전 로드하고 zone [2]에서 출구 신호를 판별합니다.
Left는 06, Right는 07, 무검출·동률은 06입니다. 두 출구를 연속 주행하지 않습니다.
06 또는 07에서 바로 시작하면 선택한 단일 출구만 실행합니다.
...
실행 카탈로그는 `zones_halla_0919_path_01.yaml`~`07.yaml`을 사용합니다.
zone [3]은 0919 CSV 그대로 경로 03 idx 57~116이며 신호등·정지선 감지와 최대 1m/s를 적용합니다.
경로 05·06·07의 옛 신호등 zone [3]은 제외했습니다. 기존 2.5m preview 규칙은 유지합니다.
zone [4]는 경로 03 idx 117~145의 CSV 범위이며 기존 접근속도 0.5m/s를 적용합니다.
zone [2]는 경로 05 idx 300~316의 출구 판별 구역입니다.
state=1 T 주차(03 idx 145), state=4 회피(04 idx 77), state=5 정지(05 idx 70)를 사용합니다.
CSV에서 제거된 경로 04 idx 185의 평행주차 지점도 실행 설정에서 제거했습니다.
...
일반 주행은 GPS를 사용하고 라인으로 전환하지 않는다. 회피 후 GPS 복귀 조건은 유지한다.
