...
class RecoveryState(str, Enum):
    NORMAL = 'NORMAL'
    WAIT_REVERSE_DELAY = 'WAIT_REVERSE_DELAY'
    ...
    WAIT_AVOIDANCE = 'WAIT_AVOIDANCE'
    FAULT_STOP = 'FAULT_STOP'


def age_exceeds_timeout(age_sec, timeout_sec):
    """Treat missing input or an age above the configured limit as stale."""
    return age_sec is None or float(age_sec) > float(timeout_sec)
...
def target_ref_has_valid_points(message):
    """Return true when a TargetRef contains at least one finite point."""
    if message is None or not message.ref_points:
        return False
    return all(
...
class ReverseRecoveryController:
    ...
    def __init__(
        ...
        *,
        ...
        self.reverse_performed_this_episode = False
        self.avoidance_handoff_pending = False
    ...
    def _snapshot(self, now_sec, output_override):
        ...
        return {
            ...
            'reverse_performed_this_episode': (
                self.reverse_performed_this_episode),
            'waiting_for_avoidance': (
                self.state == RecoveryState.WAIT_AVOIDANCE),
        }
    ...
    def _start_stop_hold(self, now_sec, reason, avoidance_eligible=False):
        if avoidance_eligible:
            self.avoidance_handoff_pending = True
    ...
    def update(
        ...
        *,
        ...
        mgm_state,
        avoid_status_fresh,
        avoid_obstacle_detected,
        avoid_avoidable,
        mgm_target_fresh,
        mgm_ref_points_valid,
    ):
        ...
        if not estop_active:
            ...
            if self.state == RecoveryState.WAIT_AVOIDANCE:
                if not (
                    avoid_status_fresh
                    and avoid_obstacle_detected
                    and avoid_avoidable
                    and mgm_target_fresh
                    and mgm_state == TargetRef.STATE_AVOID
                    and mgm_ref_points_valid
                ):
                    self.last_stop_reason = 'WAITING_FOR_MGM_AVOIDANCE'
            ...
            if self.state != RecoveryState.STOP_AFTER_REVERSE:
                avoidance_eligible = bool(
                    self.reverse_performed_this_episode
                    and self.state != RecoveryState.FAULT_STOP)
                self._start_stop_hold(
                    now_sec, 'ESTOP_CLEARED', avoidance_eligible)
            stop_elapsed = now_sec - self.stop_started_at
            if stop_elapsed >= self.post_reverse_stop_hold_sec:
                if self.avoidance_handoff_pending:
                    self.state = RecoveryState.WAIT_AVOIDANCE
                    self.avoidance_handoff_pending = False
                    self.last_stop_reason = 'WAITING_FOR_MGM_AVOIDANCE'
                    return self._snapshot(now_sec, 0.0)
        ...
        if rear_scan_timeout:
            ...
            return self._snapshot(now_sec, 0.0)
        if self.state == RecoveryState.WAIT_AVOIDANCE:
            self.state = RecoveryState.WAIT_REVERSE_DELAY
        ...
        if self.state == RecoveryState.WAIT_REVERSE_DELAY:
        ...
        if self.state in (
            RecoveryState.WAIT_REVERSE_DELAY,
            ...
            self.reverse_performed_this_episode = True
            self.last_stop_reason = ''
...
class ReverseRecoveryNode(Node):
    ...
    def __init__(self):
        ...
        self.avoid_obstacle_detected = False
        self.avoid_avoidable = False
        self.avoid_generation = 0
        self.mgm_target_generation = 0
        self.avoid_generation_at_wait_start = None
        self.mgm_generation_at_wait_start = None
        ...
        self.create_subscription(
            AvoidStatus, '/perception/avoid', self._avoid_callback, 1)
        self.create_subscription(
    ...
    def _current_inputs(self):
        ...
        waiting_baseline_set = bool(
            self.avoid_generation_at_wait_start is not None
            and self.mgm_generation_at_wait_start is not None)
        return {
            'estop_active': self.estop_active,
            'front_obstacle_present': self.front_obstacle_present,
            'front_scan_timeout': bool(
                self.status_scan_timeout
                or age_exceeds_timeout(
                    front_age, self.front_scan_timeout_sec)),
            'rear_scan_received': bool(rear_received and self.rear_scan_valid),
            'rear_scan_timeout': bool(
                age_exceeds_timeout(
                    rear_age, self.rear_scan_timeout_sec)),
            'rear_clear': bool(self.rear_scan_valid and self.rear_clear),
            'status_fresh': bool(
                not age_exceeds_timeout(
                    status_age, self.status_stale_timeout_sec)),
            'mgm_state': (
                None if self.latest_mgm_ref is None
                else int(self.latest_mgm_ref.state)),
            'avoid_status_fresh': bool(
                waiting_baseline_set
                and self.avoid_generation
                > self.avoid_generation_at_wait_start),
            'avoid_obstacle_detected': self.avoid_obstacle_detected,
            'avoid_avoidable': self.avoid_avoidable,
            'mgm_target_fresh': bool(
                waiting_baseline_set
                and self.mgm_target_generation
                > self.mgm_generation_at_wait_start),
            'mgm_ref_points_valid': target_ref_has_valid_points(
                self.latest_mgm_ref),
    ...
    def _evaluate(self, publish_always=False):
        ...
        snapshot = self.controller.update(
            ...
            **{name: inputs[name] for name in (
                ...
                'mgm_state', 'avoid_status_fresh',
                'avoid_obstacle_detected', 'avoid_avoidable',
                'mgm_target_fresh', 'mgm_ref_points_valid')})
        if (
            previous_state != RecoveryState.WAIT_AVOIDANCE.value
            and snapshot['state'] == RecoveryState.WAIT_AVOIDANCE.value
        ):
            self.avoid_generation_at_wait_start = self.avoid_generation
            self.mgm_generation_at_wait_start = self.mgm_target_generation
        elif (
            previous_state == RecoveryState.WAIT_AVOIDANCE.value
            and snapshot['state'] != RecoveryState.WAIT_AVOIDANCE.value
        ):
    ...
    def _avoid_callback(self, message):
        self.avoid_obstacle_detected = bool(message.obstacle_detected)
        self.avoid_avoidable = bool(message.avoidable)
    ...
    def _publish_status(self):
        ...
        message.data = json.dumps({
            ...
            **self.rear_diagnostics,
            ...
            'reverse_performed_this_episode': (
                snapshot['reverse_performed_this_episode']),
            'waiting_for_avoidance': snapshot['waiting_for_avoidance'],
            'mgm_state': inputs['mgm_state'],
            'avoidance_ready': bool(
                inputs['avoid_status_fresh']
                and inputs['avoid_obstacle_detected']
                and inputs['avoid_avoidable']
                and inputs['mgm_target_fresh']
                and inputs['mgm_state'] == TargetRef.STATE_AVOID
                and inputs['mgm_ref_points_valid']),
            'avoid_generation': self.avoid_generation,
            'avoid_generation_at_wait_start': (
                self.avoid_generation_at_wait_start),
            'avoid_status_fresh': inputs['avoid_status_fresh'],
            'avoid_obstacle_detected': self.avoid_obstacle_detected,
            'avoid_avoidable': self.avoid_avoidable,
            'mgm_target_generation': self.mgm_target_generation,
            'mgm_generation_at_wait_start': (
                self.mgm_generation_at_wait_start),
            'mgm_target_fresh': inputs['mgm_target_fresh'],
            'mgm_ref_points_valid': inputs['mgm_ref_points_valid'],
            'input_mgm_v_ref': (
                None if self.latest_mgm_ref is None
