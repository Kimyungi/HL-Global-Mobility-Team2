...
적색 근거를 이어간다. 이 YOLO 근거와 별개로 아래 템플릿 조건도 적용한다.
...
추가로, 현재 유효한 template 박스의 점수가 소수 둘째 자리 반올림 시
`1.00`으로 표시되면 HSV 색상이나 이전 YOLO 적색 확인 없이 적색 관측으로
사용한다. 예를 들어 0.999는 포함되고 0.9949는 포함되지 않는다. 같은 프레임의
녹색 관측보다 이 적색 근거가 우선하며, 최근 5프레임 중 3회 확정 조건은 유지한다.
유효한 template 박스가 없는 프레임에서는 이전 template 점수로 투표하지 않는다.
진단 출력의 색상 근거는 `template_1.00_red`로 기록한다.
