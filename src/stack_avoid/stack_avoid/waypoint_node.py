...
from fma_interfaces.msg import AvoidStatus, GpsPath, RefPoint, MgmState
...
class WaypointAvoidNode(Node):
    def __init__(self):
        ...
        origin_csv = str(self.declare_parameter('route_origin_csv', csv).value)
        self.origin = load_waypoints_csv(origin_csv)[0]
    ...
    def load_route(self, csv):
        points, yaws = load_waypoints_csv(csv, include_yaw=True)
        if yaws is None:
            raise ValueError('waypoint avoidance requires CSV yaw on every point')
        self.route = PathEngine(points, waypoint_yaws=yaws, origin_latlon=self.origin)
        self.route_csv = str(FilePath(csv).resolve())
    ...
    def on_gps(self, msg):
        if not self.fresh_stamp(msg.reference_stamp):
            self.gps, self.gps_received = msg, time.monotonic()
            return
        route = msg.route
        if route.enabled and not route.connecting:
            identity = (route.sequence_id, route.instance_id, route.index, route.waypoint_csv)
            if identity != self.route_identity:
                try:
                    self.load_route(route.waypoint_csv)
                except (OSError, ValueError) as error:
                    self.gps = None
                    self.diagnostic('invalid route: '+str(error))
                    return
    ...
    def tick(self):
        ...
        gps_ok = (self.gps is not None and now-self.gps_received <= self.stale_s and
                  self.fresh_stamp(self.gps.reference_stamp) and self.gps.position_valid and
                  self.gps.vehicle_heading_valid and not self.gps.route.connecting and
                  self.gps.fix_quality == 4 and
        ...
        if not owner:
            reason = 'waiting for upper AVOID authority'
        ...
        msg.scan_valid = bool(self.cloud_stamp is not None and self.fresh_stamp(self.cloud_stamp))
        if not reason and msg.points:
            # The oldest contributing acquisition bounds this preview's lifetime.
            msg.reference_stamp = min((self.gps.reference_stamp, self.cloud_stamp, tf.header.stamp),
                                      key=lambda stamp: (stamp.sec, stamp.nanosec))
        else:
            # Revised v2 does not use legacy TTC to stop: withhold invalid geometry.
            msg.points = []
