...
| 점 | station | 전역 위치 | yaw |
|---|---|---|---|
| (1) | s₀−3.0 | r(s₀−3.0) | ψ(s₀−3.0) |
| (2) | s₀ | r(s₀)+d n(s₀) | ψ(s₀) |
| (3) | s₀+0.7 | r(s₀+0.7)+d n(s₀+0.7) | ψ(s₀+0.7) |
| (4) | s₀+3.7 | r(s₀+3.7) | ψ(s₀+3.7) |
...
기본 횡오프셋은 1.0 m, 진입/복귀 거리는 각각 3.0 m이다. 직선 단일 회피의
최대 곡률은 `6×1/3²=0.667 /m`, 최소 회전반경은 **1.5 m**로 설정된
차량 최소 회전반경 **1.15 m**를 만족한다. 그러나 장애물 station 간격이 3 m인
반대편 연결에서는 A3→B2의 길이가 2.3 m, 횡이동이 2 m이므로 최대 곡률은
`12/2.3²=2.268 /m`이다. 3 m 진입/복귀 설정이 장애물 간 연결 거리를 늘리지는 않는다.
MPC 추종 성능은 이 기하 시험에서 검증하지 않으며 순간 최대 곡률도 조향 한계 검사에 포함한다.

**인지 거리 제약:** 입력 점군의 3 m 반경 제한과 횡방향 약 1 m 배치에서는
첫 확정 검출 시 이미 P1(s₀−3 m)을 지날 수 있다. 현재 planner는 이때 첫 경로 생성을
거부한다. 플롯은 첫 장애물을 미리 알고 있는 기하 예제로 명시하고, 별도의 실제
3 m 합성 라이다 시험 결과를 JSON의 `live_first_detection`에 기록한다.
이 그림의 P4 통과를 3 m 센서만으로 전체 회피에 성공했다는 의미로 해석하면 안 된다.
