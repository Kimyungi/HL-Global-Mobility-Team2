...
실행 경로를 연결한다. **PR 병합·차량 PC 재빌드 전에는 차량에 적용되지 않는다.**
...
6. 실제 정차 상태로 **10초** 기다린다. 대기 중 밀리면 정차 확인 후 10초를 다시 센다.
7. 실제로 후진한 선택 경로 구간을 역순으로 **전진**한다. 차체 yaw와 조향 곡률은
   유지하고 속도 부호만 양수로 바꾼다. 다른 후보로 재선택하지 않는다.
8. 공통 접점에 복귀하고 실제 정차 확인 후에만 `ParkingStatus.done=true`.
   MGM의 기존 정차/CSV ACK 절차가 **04 요청 → 새 GPS generation 확인 → 04 추종**한다.

`/adas/target_ref`와 CAN 발행자는 추가하지 않았다. 기존 MGM만 최종 기준점을 발행한다.
ParkingStatus는 기존 노드에서 하나만 발행하며, 평행 주차는 기존 파이프라인을 사용한다.
0.55 m/s 전진·후진, 마지막 도킹 0.15 m/s를 사용하고 MGM은 T 주차의 속도를
`v_base` 이하로 보존한다. 기존의 속도 크기 고정으로 0.15가 2 m/s가 되는 것을 방지한다.
...
- Mission ACTIVE에서는 일반 LiDAR E-stop이 제외되는 기존 정책이 있으므로,
  두 전진 구간에는 raw 전방 LiDAR 통로 정지 검사를 추가했다. 관측 없는 통로는 unknown이다.
...
- raw dump v32: bus 배치는 같지만 T 종점/속도 의미가 바뀌므로 과거 dump는 과거 빌드로 재생한다.
- PR #104의 독립 reverse core를 포함·확장한다. 이 통합 PR을 사용하면 #104를 별도 병합할 필요가 없다.

## 검증 범위와 현장 확인

오프라인에서 두 슬롯 전체 순서, 10초 대기, 실제 CSV 원점 01/02/03, 실제 b1 FOV의
