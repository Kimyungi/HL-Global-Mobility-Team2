...
| **차선 주행 중 좌우 위빙 (주기 20초 안팎, 헤딩 ±10°)** | GPS 구간에는 없고 **LANE에서만** 나타나며 긴 구간일수록 커진다(실측: 5.5s 구간 3.67° ↔ 72s 구간 9.22°, 같은 run의 GPS는 6.07°). 원인 둘 다 **ref[0] 거리**다 — ① `points_x_start`가 카메라 최소 가시거리 2.5m라 차선 ref[0]이 평균 2.03m로 조향 응답 밴드(0.8~1.8m) 밖이고, 그래서 명령→조향 지연이 0.60s(GPS는 1.18m에서 0.18s)다. ② `ref_point0_min_confidence` 게이트가 걸렸다 풀렸다 하며 ref[0]이 1.8↔2.5m로 0.70m씩 계단 점프 → **루프 이득 자체가 진동** (120초에 82회, 평균 1.5초마다) | ②는 **수정 완료** — `ref_point0_min_confidence` 0.5→**0.30**(launch 인자), 미적용 33.1%→1.1%. ①은 미적용: `ref_point0_lookahead_m` 1.8→1.2 (GPS의 1.18m가 지연 0.18s로 실증된 값). **한 번에 하나씩** 바꿀 것 — 둘을 같이 바꾸면 어느 쪽이 들었는지 못 가린다 |
