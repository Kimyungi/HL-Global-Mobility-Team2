...
### 5-1c. ⚠ v_base 0.6 → 1.0 (2026-08-17, 미검증) — 시험 순서

**횡방향은 준비됐다. 막는 것은 종방향이었고, 그래서 안전값을 같이 올렸다.**

정지 성능 실측 (run_0817_032728 × rec1_024, 정지 이벤트 **1건뿐** — 표본이 얇다):
```
v0 0.591 m/s → 반응지연 0.13s → 감속 0.94 m/s² → 0.76s / 0.314m
프로파일  0.59 0.57 0.54 0.51 0.45 0.36 0.26 0.13 0.01
감지 지연  /scan 9.9Hz × 확정 3프레임 = 0.30s
필요거리 = 0.303·v + 1.19·(0.13·v + v²/(2·0.94))
```
| v | 감지 | 제동 | 합계 | 종전 문턱 0.70m | 신규 1.20m |
|---|---|---|---|---|---|
| 0.6 | 0.18 | 0.31 | **0.49m** | ✅ 여유 0.21 | ✅ |
| 0.8 | 0.24 | 0.53 | **0.77m** | ❌ | ✅ |
| 1.0 | 0.30 | 0.79 | **1.09m** | ❌ **부딪힌다** | ✅ 여유 0.11 |

같이 올린 값 — **셋은 세트다. 하나만 되돌리지 말 것**:

| 값 | 0.6 시절 | 1.0 | 위치 |
|---|---|---|---|
| `v_base` | 0.6 | **1.0** | `adas_mgm/config/params.yaml` |
| `target_speed_mps` | 0.6 | **1.0** | `stack_avoid/config/params.yaml` |
| `estop_on/off_distance_m` | 0.70/0.80 | **1.20/1.35** | `stack_estop/stack_estop/node.py` |
| `ttc_stop` | 0.8s | **1.3s** | `adas_mgm/config/params.yaml` |

⚠ `estop_on/off_distance_m` 은 **생성자에서 1회만** 읽는다 — `ros2 param set` 이 안 먹으니
바꾸려면 **재실행**해야 한다. (`v_base`·`ttc_stop`·`target_speed_mps` 는 런타임 변경 가능.)

**시험 순서** — 한 단계씩, 각 단계에서 정지 이벤트를 **3~4건** 만들어 제동 모델을 굳힐 것:
1. **정지거리부터 재측정.** 직선에서 콘 하나 두고 0.6 → 0.8 → 1.0 순으로 접근 정지.
   실제 정지거리가 위 표를 넘으면 거기서 멈추고 문턱을 더 올린다.
   (`ros2 param set /adas_mgm_node v_base 0.8` 로 단계 조절, estop 문턱은 재실행)
2. **GPS 단독** (`gps_only:=true`, 장애물 없음) — 횡오차·채터링이 0.6 때(평균 0.074m,
   Δ기하δ 0.46°)에서 얼마나 나빠지는지. 서보 지연 0.19s × 1.0m/s = 0.19m 이동이라
   횡오차는 커지는 게 정상이다. 발산만 아니면 통과.
3. **회피 단독** (이기돈) — **이번 상향의 가장 큰 미검증 구간**. TTC·측방 여유가
   0.6 m/s 에서 튜닝된 값이다. 정지 트리거가 1.2m 로 멀어져 **회피로 피할 것을
   estop 이 먼저 세우는지** 를 특히 볼 것.
4. **통합** — 그 다음.

**회피 쪽에서 같이 움직인 값** (`stack_avoid/config/params.yaml`) — 속도에 **기계적으로**
매달린 것만 손댔다. 기하 튜닝값(`lateral_margin_m`·`offset_max_m`·`depth_band_m`)은 안 건드렸다:

| 값 | 전 | 후 | 왜 |
|---|---|---|---|
| `detect_range_m` | 3.0 | **5.0** | ① 경고시간 5.0s 복원(3.0m 는 1.0m/s 에서 3.0s) ② estop 문턱 상향이 뺏은 회피 가용거리 복구 (3.0−0.70=2.30m → 5.0−1.20=3.80m) |
| `clear_gap_max_m` | 3.0 | **5.0** | detect_range 에 유계된 값 — 안 따라 올리면 통과거리 과소평가로 2026-08-14 측면충돌 2회 재현 |
| `target_rate_limit_mps` | 3.0 | **5.0** | 프레임간 정당한 목표 이동이 v 에 비례 (10Hz 에서 0.06m → 0.10m). 안 올리면 진짜 변화까지 깎인다 |

⚠ **`ttc_stop_s` 1.5 는 일부러 안 건드렸다 (이기돈 판단 필요).** 시간 게이트라 속도에
불변처럼 보이지만 실제 필요 제동시간이 0.77s(0.6) → 1.19s(1.0) 로 늘어 여유가
0.73s → 0.31s 로 줄었다. 여유를 되돌리려면 2.0s 인데, **올리면 "회피 불가" 판정이
늘어 AVOID 대신 정지가 많아진다** — 시연 시나리오가 안 돌 수 있다. 기능/안전 맞교환이라
회피 단독 시험에서 이기돈이 정할 것.

⚠ **회피 기하는 이 로그로 못 튜닝한다.** 회피 구간을 조향각으로 적분해 봤지만
(0.6 m/s, 12s/7.1m) 헤딩 적분 드리프트가 커서 "종방향 몇 m 만에 횡 0.46m 를 버는가"가
안정적으로 안 나온다. 위 세 값은 **속도 비례 관계에서만** 나온 것이고, 회피가 실제로
성립하는지는 회피 단독 시험으로만 확인된다.

**되돌리는 법**: `git revert <이 커밋>` 하나면 일곱 값이 같이 돌아간다.

**아직 안 건드린 것 (의도적)**:
- `v_accel_zone` 1.0 — base 와 같아져 **가속구간이 무의미해졌다**. 1.0 초과 속도는
  실측이 없어서 올리지 않았다. 1.0 이 안정되면 그때 검토.
- `v_narrow` 0.2 — §3 ① "감속하면서 조향 금지" 와 이미 충돌하는데 낙차가 0.4 → 0.8 m/s
  로 두 배가 됐다. 이기돈과 정책 결정 필요 (CLAUDE.md §3 ① 의 미해결 항목).

### 5-2. ⚠ 조향 PI 이후 **실현율 곡선 재측정**이 최우선

**⚠ 2026-08-17 반증 신호 — "2.5m 넘으면 붕괴"가 뒤집혔을 수 있다.**
run_0817_032728 에서 실현 이득(실제δ / PC 기하δ)을 ref[0] 거리로 나눠 보면 **거리가
멀수록 좋아진다** — §3 ③ 의 pre-PI 법칙과 정반대다:

| d0 | 1.50~1.90m | 1.90~2.10m | 2.10~2.40m | 2.40~3.50m |
|---|---|---|---|---|
| 실현 이득 | 54.8% | 53.4% | **71.7%** | **76.1%** |
| n (스테이트) | 3514 (GPS) | 8276 (차선) | 1277 (차선) | 557 (차선) |

**단, 이건 교란된 값이다** — 먼 구간은 거의 전부 차선이고, d0 가 큰 순간은 경로가
완만한 순간이기도 하다. 인과가 아니라 상관일 수 있다. 그래서 **속도를 올리면서
lookahead 는 같이 올리지 않았다**: 1.0 m/s 첫 run 은 속도 하나만 바뀐 상태로 봐야
하고, 1.8m 에서 미리보기 1.8s = 서보 지연 0.19s 의 9배라 급하지 않다. 거리는 아래
재측정으로 인과를 확정한 뒤 올린다. 올린다면 GPS 는 `ref_lookahead_m` 이 아니라
**`rejoin_target_min_m`/`max_m`** 이 실효 거리다(둘 다 1.8 = 하한이 이긴다).
