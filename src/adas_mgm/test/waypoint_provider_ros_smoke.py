...
from fma_interfaces.msg import (CanHealth, EstopRequest, EstopRecovery, GpsPath,
    LanePath, AvoidStatus, MgmState, RefPoint, TargetRef, VehicleVector, ZoneContext, TrafficStop)
...
def main():
    ...
    kinds = {'/perception/gps_path': GpsPath, '/perception/lane_path': LanePath,
        ...
        '/perception/traffic_stop': TrafficStop, '/perception/avoid': AvoidStatus}
    ...
    params = dict(revised_v2_enabled=True, wait_go=True, lidar_estop_enabled=False,
        ...
        avoid_unblended=True, zone_enter_confirm_samples=5,zone_exit_confirm_samples=5, avoid_zone_only=True)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            gps.avoid_zone=True
            avoid.scan_valid=True;avoid.obstacle_detected=True;avoid.avoidable=True
            avoid.ttc=1.e9;avoid.v_suggest=1.;avoid.points=[RefPoint(x=.8,y=.6,yaw=.2,curvature=.1)]
            expect(lambda s,r:s.avoidance==1 and r.v_ref>0 and len(r.ref_points)==1 and abs(r.ref_points[0].x-.8)<1.e-5 and abs(r.ref_points[0].y-.6)<1.e-5,
                   'new provider 1m point reaches MGM unchanged')
            avoid.points=[];avoid.avoidable=False;avoid.ttc=0.;avoid.v_suggest=0.
            expect(lambda s,r:s.avoidance==1 and r.v_ref==0 and s.reference_motion_blocked,
                   'invalid fixed geometry stops without leaving AVOID')
