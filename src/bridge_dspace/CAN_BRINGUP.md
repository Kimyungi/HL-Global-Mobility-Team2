...
>
> **이번 이관은 1단계다**: 와이어 포맷만 FD 로 바꾸고 페이로드는 8바이트를 유지한다.
> FD 의 64바이트를 실제로 쓰는 재포장은 2단계로 분리했다 (PROTOCOL.md "2단계" 절).
> 두 단계를 한 번에 하면 안 붙었을 때 원인이 6갈래로 갈린다.
>
...
| n_points는 헤더에서 읽기 (실운용 20점 — 확장 대비 가변 필드) | PROTOCOL.md TX 절 |
| watchdog: `0x100` counter 30ms 미갱신 → v_ref=0, 조향 유지 | CLAUDE.md §3 |
| 메시지 길이(DLC) = **정확히 8바이트** (FD 패딩 금지) | PROTOCOL.md §공통 |
...
✅ `TX 프레임/헤더 = 21.00` (20점 + 헤더)은 v2와 동일해야 한다 — 1단계는 프레임 수를
바꾸지 않는다. 이 값이 변했으면 뭔가 잘못된 것이다.
