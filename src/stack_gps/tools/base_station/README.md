...
>
> **⚠ 베이스를 옮기기 전에 `read_base_position.py` 를 먼저 돌릴 것.** 좌표의 사본은
> F9P 플래시와 이 블록 딱 둘뿐이고, `measure_base_position.py` 는 결과를 화면에만
> 찍는다. 여기 적어 두지 않은 채 새 지점을 `setup_base.py` 로 쓰면 옛 좌표는 복구할
> 데가 없고, 그 지점에서 기록한 코스 CSV 가 통째로 못 쓰게 된다.
>
>     python3 read_base_position.py          # 현재 저장된 좌표 + README 붙여넣을 줄 출력
>
> **좌표 3개만으로 되는가 — 소프트웨어는 되고, 물리는 조건이 붙는다.**
> `setup_base.py --lat --lon --height` 가 나머지 설정(1Hz·UART2 RTCM 6종)까지 같이
> 다시 쓰므로 수신기 쪽은 그 3개면 완전 복원된다. 다만 베이스는 **자기가 어디 있는지
> 모르고 우리가 말해 준 값을 믿는다** — 안테나를 그때와 다른 자리에 세우면 로버 위치
> 전체가 그 차이만큼 밀린다. **삼각대 높이도 포함**이다(타원체고가 그만큼 틀어진다).
> 그래서 좌표 옆에 **어디에 어떻게 세웠는지**를 같이 적어 둘 것.
>
> 코스 재사용에 필요한 건 절대 정확도가 아니라 **일관성**이다. 그때 측량이 진짜
> 위치에서 20cm 틀렸더라도 같은 좌표를 선언하고 같은 자리에 세우면 그때와 같은
> 프레임이 재현돼 CSV 가 맞는다. 반대로 같은 자리에서 **재측량하면** 값이 cm 단위로
> 달라져 코스가 그만큼 밀린다 — 그래서 옛 코스를 쓸 거면 재측량이 아니라 **그때
> 숫자를 그대로 다시 넣는 것**이 정답이다.
>
> 옛 좌표를 잃어버렸을 때 회수 경로: ① 그 지점을 설정했던 PC 의 셸 히스토리
> (`grep setup_base ~/.bash_history` — 실행 커맨드에 좌표가 그대로 들어 있다)
> ② 그 EVK 를 가져와 `read_base_position.py`
>
