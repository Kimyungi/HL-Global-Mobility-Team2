...
> **확정 베이스 좌표 — 위치별 좌표와 전환 절차는 [`BASE_LOCATIONS.md`](BASE_LOCATIONS.md) 에서 관리한다.**
> 수신기는 한 번에 한 위치만 저장하므로, 어느 지점에 있는지에 따라 그 지점 좌표로
> `setup_base.py` 를 다시 돌리고 **그 지점에서 기록한 웨이포인트만** 쓴다.
>
> **⚠ 베이스를 옮기거나 재측량하기 전에 `read_base_position.py` 를 먼저 돌릴 것.**
> 좌표 사본은 F9P 플래시와 `BASE_LOCATIONS.md` 딱 둘뿐이고, `measure_base_position.py`
> 는 결과를 화면에만 찍는다. 표에 적어 두지 않은 채 새 지점을 `setup_base.py` 로 쓰면
> 옛 좌표는 복구할 데가 없고, 그 지점에서 기록한 코스 CSV 가 통째로 못 쓰게 된다.
>
>     python3 read_base_position.py     # 현재 저장된 좌표 + 표에 붙여넣을 줄 출력
