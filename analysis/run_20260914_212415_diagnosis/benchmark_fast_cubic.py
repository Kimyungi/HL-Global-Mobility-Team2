...
from fast_cubic_prototype import cubic_connector as fast,basis
...
for gap in [3.3,2.,.7]:
 ...
 for name,func in [('baseline',old),('prototype',fast)]:
  ...
  for i in range(7):
   ...
   if name=='prototype' and i==0:basis.cache_clear()
...
print(json.dumps(result,indent=2));Path(__file__).with_name('fast_cubic_results.json').write_text(json.dumps(result,indent=2))
