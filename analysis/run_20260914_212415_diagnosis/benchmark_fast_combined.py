...
from fast_footprint import footprint_clear
old_fp=mod.footprint_clear
...
for gap in [3.3,2.,.7]:
 ...
 for name,func in [('baseline',old),('prototype',fast)]:
  mod.cubic_connector=func;mod.footprint_clear=old_fp if name=='baseline' else footprint_clear; times=[];cold=[]
  for i in range(7):
...
print(json.dumps(result,indent=2));Path(__file__).with_name('fast_combined_results.json').write_text(json.dumps(result,indent=2))
