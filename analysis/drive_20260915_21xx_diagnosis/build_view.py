...
for p in sorted(OUT.glob('*_decoded.csv')):
 ...
 first=next((r['t'] for r in rows if r['top']==1), None)
 first_window=first if first is not None else times[0]
 rev=None;start=None
 for r in rows:
  if first is None or r['t']<first:continue
 ...
 for g in gps:
  t=float(g['stamp_s'])
  if not first_window-2<=t<=end:continue
 ...
 events=[];prev=None
 for r in rows:
  if not first_window-2<=r['t']<=end:continue
  key=tuple(r[k] for k in ['top','source','avoid','safety','route_index'])
  if key!=prev:
   label=state(r,rev is not None and r['t']>=rev)
   events.append(dict(t=r['t'],time=clock(r['t']),label=label,route=['04'][int(r['route_index'])],text=f"{label} · route {int(r['route_index'])+1}"));prev=key
 ...
 routes={}
 origin=read(ROOT/'src/stack_gps/waypoints/waypoints_halla_reference_path_04.csv')[0]
 lat0,lon0=float(origin['lat']),float(origin['lon'])
 for rid in ['01','03','04']:
  routes[rid]=[[(float(x['lon'])-lon0)*111320*math.cos(math.radians(lat0)),(float(x['lat'])-lat0)*111320] for x in read(ROOT/f'src/stack_gps/waypoints/waypoints_halla_reference_path_{rid}.csv')]
 summary=dict(name=name,first_go=clock(first),manual_start=clock(rev),manual_t=rev,drive_s=round(len(active)*.01,2),state_seconds={k:round(v*.01,2) for k,v in phases.items()},fix_counts=dict(collections.Counter(g['quality'] for g in driving_g)),heading_counts=dict(collections.Counter(g['heading_src'] for g in driving_g)),max_cross=round(max((r['cross'] for r in active),default=0),3),transition_check_count=len(transitions),blocked_seconds=round(sum(bool(r['blocked']) for r in active)*.01,2),route_points_behind_seconds=round(sum(r['source']==1 and r['gps_point_x']<0 and r['v_ref']>0 for r in active)*.01,2))
...
for j,run in enumerate(result):
 ...
 for rid,pts in run['routes'].items():
  ax.plot([p[0] for p in pts],[p[1] for p in pts],color='#334155' if rid=='04' else '#cbd5e1',ls='--',lw=2,label=('selected CSV ' if rid=='04' else 'context CSV ')+rid)
 for label in C:
 ...
 ax.scatter(0,0,c='#334155',marker='s',s=50);ax.annotate('선택 Path 04 시작', (0,0),xytext=(8,8),textcoords='offset points',fontsize=9)
 ax.scatter(xs[0],ys[0],c='#0f172a',s=35,zorder=9);ax.annotate('GO' if run['summary']['first_go'] else 'WAIT',fs[0]['xy'],xytext=(5,8),textcoords='offset points')
 cut=next((f for f in fs if f['manual']),None)
 if cut:ax.scatter(*cut['xy'],c=C['MANUAL'],marker='X',s=100,zorder=10);ax.annotate('수동 시작\n'+run['summary']['manual_start'],cut['xy'],xytext=(10,10),textcoords='offset points',fontsize=9)
 xs += [0];ys += [0];margin=3;ax.set_xlim(min(xs)-margin,max(xs)+margin);ax.set_ylim(min(ys)-margin,max(ys)+margin);ax.set_aspect('equal');ax.grid(alpha=.2);ax.set_xlabel('East (m)');ax.set_ylabel('North (m)')
 ax.set_title(run['summary']['name'][12:18]+' 세션\nGO '+(run['summary']['first_go'] or '없음'),fontsize=12);ax.legend(fontsize=8,loc='best')
 ...
 b.axhline(0,color='#94a3b8',lw=.6);b.set_ylabel('m/s');b.set_xlabel('표시 시작부터 경과 (s)');b.grid(alpha=.2);b.legend(fontsize=8)
fig.suptitle('2026-09-15 21시대 · 차량 GPS 위치와 MGM 상태\n보라색 점선은 후진 시작 이후 수동 조작 — 자율주행 원인 분석에서 제외',fontsize=16)
