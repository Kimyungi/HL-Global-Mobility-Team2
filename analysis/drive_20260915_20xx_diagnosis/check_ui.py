...
harness='''<script>setTimeout(()=>{let count=0;const ok=(v,m)=>{if(!v)throw Error(m);count++};try{ok(DATA.length===3,'sessions');for(let n=0;n<3;n++){document.getElementById('run').value=n;document.getElementById('run').onchange();ok(frames.length>100,'frames');ok(document.getElementById('badge').textContent.length>0,'badge');ok(document.getElementById('events').children.length>0,'events');document.getElementById('slider').value=frames.length-1;document.getElementById('slider').oninput();ok(frames[idx].manual,'manual endpoint');document.getElementById('manual').checked=false;document.getElementById('manual').onchange();ok(frames.every(f=>!f.manual),'manual hidden');document.getElementById('manual').checked=true;document.getElementById('manual').onchange();document.getElementById('events').firstChild.click();ok(idx>=0&&idx<frames.length,'jump');}document.body.insertAdjacentHTML('beforeend','<pre id="ui-check">PASS '+count+'</pre>')}catch(e){document.body.insertAdjacentHTML('beforeend','<pre id="ui-check">FAIL '+e.stack+'</pre>')}},250)</script>'''
