...
2026-09-19 · `integration/v2_main` · **스테이트 v09.17**

현재 용인 전체 주행용 런북입니다. `RUN_BOOK_HALLA_FINAL.md`의 터미널별 절차를
현재 용인 CSV·주차 카탈로그·출구 판단에 맞게 갱신했습니다.
기준 구현은 `2ff28b9`이며, 아래 하드웨어 명령은 문서 작성 중 실행하지 않았습니다.
...
`parking_ref_01.csv`·`02.csv`, `parking_waypoint*.csv` 및 한라 CSV는 이 용인 카탈로그에서 사용하지 않습니다.
GPS·주차·회피는 선택한 시작 CSV의 첫 위도·경도를 공통 원점으로 사용합니다.

## 2. 코드 변경 후 준비 — 주행 런처 종료 상태
...
GPS 표시를 Ctrl+C로 닫은 뒤, 다음 중 **한 가지**로 전체 스택을 준비합니다.
`prepare`와 `drive`는 같은 용인 전체 주행 런처입니다.

### 시작 경로·속도를 질문받는 기본 명령
...
준비 완료와 실제 주행 참조·IMU·주차 피드백 유효성은 별도로 판단합니다.
상태 표시를 Ctrl+C로 닫고 출발을 인가합니다.

```bash
scripts/v2 go
```

`출발 인가 완료 (MGM 확인)`을 확인합니다. 현재 go 도구에는 옛 `/scan` remap이 필요하지 않습니다.
신호등 판단 메시지는 zone [3] 안에서만 발행하므로 일반 출발 명령에 `--require-traffic`을 붙이지 않습니다.

주행 중 상태를 다시 확인할 때도 `scripts/v2 state`를 사용합니다.
...
| 03 | [3] 160~177 → [1] 178~222, 270~336 → state=4: 336 → [1] 463~496 → [3] 565~579 → [1] 580~675 → [4] 676~683 → state=1: 683 |
...
후방 거리 성공 판정의 로컬 보류 차이는 §2를 참고합니다.
