...
**이 PC의 보류 변경:** 주차 실행기·테스트·문서와 주차 참조 경로 03/04에
별도 미커밋 수정이 남아 있습니다. 기본 no-parking에서는 T·평행 주차 참조 실행을
끄므로 해당 주차 동작은 사용하지 않습니다. 기존 `course:=yongin`을 선택할 때는
로컬 주차 변경을 별도로 검토해야 합니다.
