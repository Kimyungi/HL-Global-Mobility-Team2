...
- ESTOP: zone [6], 전방 5m·한 군집 5점·점 간격 5cm, 새 스캔 3회 확인,
  유효 실속도 정차 7초 후 해제, 해당 구역 이탈 전 재진입 금지, 후진 미사용.
- 주차: 3초간 위치 미확정 시 T자 01·평행 03. T자는 1-1 입차→1-2 출차 또는
  2-1 입차→2-2 출차를 선택 위치에 맞게 사용합니다. 평행 state=2는 유지합니다.
- 신호등: 노출 보정 -9 + 픽셀 밝기 70% 후처리. 마지막 미션 카메라에는 적용하지 않습니다.
...
| `reference_motion_blocked`, `active_safe_stop_reasons` | GO 후 정차 원인 확인 |

현재 revised v2 준비 조건은 **라이다 4대 정상 AND GPS FIXED·유효 경로**입니다.
카메라 영상만으로 GPS 준비 조건을 대신할 수 없습니다.
준비 완료와 실제 이동에 필요한 GPS 헤딩·선택 참조·차속 피드백 유효성은 별도입니다.
상태 표시에서 Ctrl+C를 누르면 표시만 종료됩니다.
`scripts/v2 state`는 `/adas/mgm_state`만 표시합니다. 실제 목표속도는 부록 B의
환경 설정 후 `ros2 topic echo /adas/target_ref --field v_ref`로 별도 확인합니다.
GO 전 목표속도 0은 정상입니다.
...
위 명령은 신호등 밝기 필터 0.7도 기본 적용합니다. 원복 시험 시 `traffic_image_brightness_scale:=1.0`을 추가합니다.
...
신호등 카메라는 노출 보정 `traffic_exposure_compensation=-9`에 더해
`traffic_image_brightness_scale=0.7`로 촬영 후 픽셀 밝기를 70%로 낮춥니다.
YOLO·색상 판단·추적·정지선 및 신호등 디버그 영상에 같은 보정을 적용합니다.
마지막 미션은 `/perception/lane_image_raw` 원본을 사용하므로 이 보정을 적용하지 않습니다.
`traffic_image_brightness_scale:=1.0`이면 후처리만 해제합니다. 설정 변경은 런처 재시작 후 적용됩니다.
...
장애물 후보는 `/unified_lidar/cloud`에서 차량 기준 반경 3m 안의 점을 사용하며,
웨이포인트 기준 좌우 거리 **0.75 ± 0.25m (0.50~1.00m)** 대역에서 판단합니다.
최소 3점 군집을 2개 연속 입력에서 확인합니다. 감지 대역과 별개로 회피 경로의
횡이동 설정은 1.0m, preview는 1.5m입니다.
...
state=5의 5초 대기는 유효 차속으로 실제 정차를 확인한 동안 계산합니다.
...
정차 확인 후 3초 동안 위치를 확정하지 못하면 T자 01·평행 03을 선택합니다.
3초 전에 확정한 선택은 유지합니다. T자 01은 1-1 입차→1-2 출차,
T자 02는 2-1 입차→2-2 출차입니다. 평행은 기존 03·04 판단과 state=2를 유지합니다.
후진 경로 끝 도달 후 실제 정차 확인도 주차 성공 조건입니다.
완료 후 3초 정차하고 탈출하며, 탈출 완료와 다음 CSV 인계 때문에 별도 정차 시간을 추가하지 않습니다.
함께 보관한 주차 변경과 기본 코스 구분은 부록 A를 참고합니다.
...
- 해제 후 이전 상태로 복귀하고 해당 스테이션 이탈 전 재진입을 금지합니다.
  다른 주행·참조 정지 조건이 남아 있으면 실제 이동은 제한될 수 있습니다.
- 과거 estop_recovery 실행기는 런처에서 실행하지 않으며, 옛 후진·done 명령도 사용하지 않습니다.

회피 [5]는 경로 03, ESTOP [6]은 경로 04에 있어 현재 CSV에서 겹치지 않습니다.
ESTOP이 활성화되면 회피보다 우선하여 목표속도 0을 적용합니다. [5]에서 ESTOP이
회피 실패의 대체 정지 기능으로 동작하는 것은 아닙니다. 회피 참조 무효·경로 막힘은
회피 참조 게이트가 정지시킵니다. 두 구역 밖에서는 이 두 기능으로 장애물을 처리하지 않습니다.
...
| FIXED인데 주행 참조가 무효 | IMU/헤딩·GPS 참조 신선도·차속 수신 확인 |
| GO 후 목표속도 0 | `reference_motion_blocked`, 정지 사유, state=5 대기, 신호 상태, 경로 ACK, 상위 ESTOP |
...
## 2026-09-20 신호등 영상 밝기 후처리

`prepare/drive` 기본값은 카메라 노출 보정 `traffic_exposure_compensation=-9`와
`traffic_image_brightness_scale=0.7`이다. 촬영 후 BGR 픽셀에 0.7을 곱한 영상을
신호등 YOLO·HSV·추적·정지선 인지 및 디버그 영상에 사용한다.
전체 런처를 재시작해야 적용된다. `traffic_image_brightness_scale:=1.0`이면
후처리를 끄며, 허용 범위는 0 초과 1 이하이다. 0.7은 실제 조도(lux)나 노출 EV 값이 아니다.

마지막 미션은 `/perception/lane_image_raw` 원본을 사용하며 이 필터를 적용하지 않는다.
신호등의 `/perception/traffic_image_raw` 역시 보정 전 원본이다.
소프트웨어로 어둡게 해도 포화로 사라진 색은 복원되지 않는다. 인식 개선은 실차 확인이 필요하다.
